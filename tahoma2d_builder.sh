...
then 
	...
	dnf install -y $(dirname "$0")/opencv/opencv-4.6.0-7.el7.x86_64.rpm $(dirname "$0")/opencv/opencv-contrib-4.6.0-7.el7.x86_64.rpm $(dirname "$0")/opencv/opencv-core-4.6.0-7.el7.x86_64.rpm $(dirname "$0")/opencv/opencv-devel-4.6.0-7.el7.x86_64.rpm
...
#	fi
	...
	cd $(dirname "$0")
	cp -f scripts/tahoma2d appdir/usr/bin/
	cd $BUILDDIR
...
#	tar zcf Tahoma2D-linux.tar.gz Tahoma2D
	cd $BUILDDIR
	chmod -R ugo+rwx ../tahoma2d-build-redos
else
